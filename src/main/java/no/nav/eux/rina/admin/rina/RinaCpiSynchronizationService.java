...
//    log.info("gid0 [{} : {}]", rinaTenant.getInstitutionId(), ToStringBuilder.reflectionToString(syncInitialDocumentDto));
    ...
    resourceDtoList.sort(new Comparator<ResourceDto>() {
      ...
      public int compare(ResourceDto o1, ResourceDto o2) {
        ...
        //log.info("v2 = [{}]", v2);
        //log.info("v1.compareTo(v2) = [{}]", v1.compareTo(v2));
        return v1.compareTo(v2);
      }
    });
    String currentVersion = resourceDtoList.get(resourceDtoList.size() - 1).getVersion();
//    log.info("currentVersion from resourcesApi [{}]", currentVersion);
    Semver semver = new Semver(currentVersion);
//    log.info("semver from currentVersion [{}] and withClearedSuffixAndBuild [{}]", semver, semver.withClearedSuffixAndBuild());
...
//    log.info("upd1 [{}]", ToStringBuilder.reflectionToString(syncInitialDocumentDto));
    return syncInitialDocumentDto; //.putInitialDocumentItem("initalDocument", resourceDtoList.get(resourceDtoList.size()-1));
  }
  
  public void submitDocumentIR(SyncInitialDocumentDto dto, String defaultVersion) {
//    log.info("submitting a syn002 [{}] with defaultVersion {}", dto.getInitialDocument(), defaultVersion);
//    log.info("sub0 [{}]", ToStringBuilder.reflectionToString(dto));
...
//    log.info("SIRn [{}]", ToStringBuilder.reflectionToString(defaultVersion));
    
    try {
//      log.info("subz dto [{}]", ToStringBuilder.reflectionToString(dto));
//      log.info("subz inD [{}]", ToStringBuilder.reflectionToString(dto.getInitialDocument()));
...
//    log.info("syn002version = [{}]", syn002Version);
...
//    log.info("availableVersion = [{}]", availableVersion);
...
//    log.info("get0 [{}]", ToStringBuilder.reflectionToString(dto));
    try {
